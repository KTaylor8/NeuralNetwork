...
class network():
    ...
    def runNetwork(self):
        ...
        with open(
            ...
            for minibatch in minibatches:
                tOut = minibatch[self.layerSizes[0]]
                minibatchInputs = minibatch[0:self.layerSizes[0]]
                inputs = np.reshape(
    ...
    def makeMinibatchesList(self, dataFile):
        ...
        for minibatch in dataFile:  # each row begins as string
            minibatch = minibatch.strip()
            minibatchSplit = minibatch.split(",")
            minibatch = []
            for el in minibatchSplit:
                el = float(el)
                minibatch.append(el)
    ...
    def sigmoid(self, dotProdSum):
        ...
        return activation  # RuntimeWarning: overflow encountered in exp
